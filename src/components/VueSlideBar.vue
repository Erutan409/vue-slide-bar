    ...
    <div
        ...
        <div ref="elem" class="vue-slide-bar" :style="{height: `${lineHeight}px`}" id="slider">
          ...
          <span class="vue-slide-bar-tooltip-top vue-slide-bar-tooltip-wrap" v-if="showTooltip">
            <slot name="tooltip">
              <span class="vue-slide-bar-tooltip" :style="tooltipStyles">{{ val | valTransform }}</span>
            </slot>
          </span>
                </div>
            </template>
        ...
        return {
            ...
            computed: {
                ...
                total() {
                    if (this.data) {
                        ...
                        % (this.interval * this.multiple) !== 0
                    ) {
                        this.printError(
                            `[VueSlideBar error]: Prop[interval] is illegal,
                         Please make sure that the interval can be divisible`
                        );
                    }
            ...
            watch: {
                ...
                max(val) {
                    ...
                    this.setValue(this.limitValue(this.val));
                    this.refresh();
                },
                min(val) {
                    if (val > this.max) {
            ...
            filters: {
                valTransform(val) {
                    const transformer = app.valueTransformer;

                    if (transformer !== null) {
                        val = transformer(val, app.min, app.max);
                    }
            ...
            methods: {
                ...
                wrapClick(e) {
                    if (this.isDisabled || (!this.draggable && e.target.id === 'wrap')) {
                        return false;
                    }
                ...
                setTransform(val) {
                    const translateValue = `translateX(${val - ((this.$refs.tooltip.scrollWidth - 2) / 2)}px)`;
                ...
                limitValue(val) {
                    ...
                    const inRange = (v) => {
                        if (v < this.min) {
                            this.printError(
                                `[VueSlideBar warn]: The value of the slider is ${val},
                             the minimum value is ${this.min}, the value of this slider
                             can not be less than the minimum value`
                            );
                            ...
                            this.printError(
                                `[VueSlideBar warn]: The value of the slider is ${val},
                             the maximum value is ${this.max}, the value of this slider
                ...
                printError(msg) {
                    console.error(String(msg).trim().replace(/\s{2,}/g, ' '));
                }
            },
            beforeMount() {
                app = this;
